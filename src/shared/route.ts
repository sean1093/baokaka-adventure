 ...
 * (…/#/find, …/#/quest) and the browser back button leaves a game. No router library:
 * three routes do not justify one.
 */
export type Route = 'hub' | 'find' | 'quest';

const HASHES: Record<Exclude<Route, 'hub'>, string> = { find: '#/find', quest: '#/quest' };

export function parseRoute(hash: string): Route {
  const path = hash.replace(/^#\/?/, '').replace(/\/+$/, '');
  return path === 'find' || path === 'quest' ? path : 'hub';
}
